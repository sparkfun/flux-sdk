...
uint8_t flxDevBME68x::defaultDeviceAddress[] = {kBME68xAddressDefault, kBME68xAddressAlt1, kSparkDeviceAddressNull};

//----------------------------------------------------------------------------------------------------------
// Register this class with the system, enabling this driver during system
// initialization and device discovery.

flxRegisterDevice(flxDevBME68x);
...
bool flxDevBME68x::isConnected(flxBusI2C &i2cDriver, uint8_t address)
{
...
//
bool flxDevBME68x::onInitialize(TwoWire &wirePort)
{
...
float flxDevBME68x::read_TemperatureC()
{
    if (!_temperature)
    {
...
float flxDevBME68x::read_Humidity()
{
    if (!_humidity)
    {
        Bme68x::setOpMode(BME68X_FORCED_MODE);
...
float flxDevBME68x::read_Pressure()
{
    if (!_pressure)
    {
        Bme68x::setOpMode(BME68X_FORCED_MODE);
...
float flxDevBME68x::read_GasResistance()
{
    if (!_gasResistance)
    {
        Bme68x::setOpMode(BME68X_FORCED_MODE);
...
uint8_t flxDevBME68x::read_Status()
{
    if (!_status)
    {
        Bme68x::setOpMode(BME68X_FORCED_MODE);
