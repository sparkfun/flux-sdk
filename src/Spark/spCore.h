...
//
enum DataTypes { TypeNone, TypeBool, TypeInt, TypeFloat, TypeDouble, TypeString};
...
protected:
	...
	// reference version...
	void  addProperty(spPropertyBase& newProperty){ addProperty(&newProperty); };
...
public:
	...
	virtual DataTypes type(void) {return TypeNone;}
...
protected:
	...
	inline DataTypes _getType(std::nullptr_t *t){return TypeNone;};
	inline DataTypes _getType(bool* t){return TypeBool;};	
	inline DataTypes _getType(int* t){return TypeInt;};
	inline DataTypes _getType(float* t){return TypeFloat;};			
	inline DataTypes _getType(double* t){return TypeDouble;};
...
public:
	...
	virtual size_t save_size(void){return 0;}; // number of bytes used to persist value
	virtual bool save(spStorageBlock * stBlk)=0;
	virtual bool restore(spStorageBlock * stBlk)=0;
...
template <class T>
class spProperty: public spPropertyBase{
...
public:
	...
	size_t save_size(){ return size();  }; // sometimes save size is different than size
...
public:
	...
	DataTypes type(void){ return TypeString;};

	//----------------------------------------
	// size in bytes of this property
	size_t size(){ return _Data.size();}
	size_t save_size(){ return _Data.size() + sizeof(uint8_t);}	// string add len
...
template<typename T>
class spParamOut : public spDataCore {
...
public:
	...
	T operator()(){ return _handler_func();}	
...
template<typename T>
class spParamIn: public spDataCore {
...
public:
	...
	void operator()(spParamOut<T>& inputParam) { _inputParameter = &inputParam;}

	const T get(void)const {return _inputParameter == nullptr ? (T)0 : (*_inputParameter)();}
...
inline bool operator==(const spType& lhs, const spType& rhs){ return &lhs == &rhs; }
inline bool operator==(const spType& lhs, const spType* rhs){ return &lhs == rhs; }
inline bool operator==(const spType* lhs, const spType& rhs){ return lhs == &rhs; }
...
template<typename T>
class spContainer: public spBase{
...
public:
	...
	T* at(int i){ return _children.at(i);}

	void add(T* theChild){ _children.push_back(theChild);}
	...
	typename std::vector<T*>::iterator begin() { return _children.begin(); }
 	typename std::vector<T*>::iterator end() { return _children.end(); }
	typename std::vector<T*>::const_iterator cbegin() const { return _children.cbegin(); }
	typename std::vector<T*>::const_iterator cend() const { return _children.cend(); }
	typename std::vector<T*>::reverse_iterator rbegin()  { return _children.rbegin(); }
	typename std::vector<T*>::reverse_iterator rend()  { return _children.rend(); }
...
struct spAction: public spBase {
	...
	virtual bool loop(void){return false;}; // default is a noop
};
...
public:
	...
	// connects a member function to this spSignal
  	template <typename T>
	void call(T *inst, void (T::*func)(ArgT var)) {  
 	...
 	// connects a const member function to this spSignal
  	template <typename T>
	void call(T *inst, void (T::*func)(ArgT var) const) {
	...
	void call(void (*func)(ArgT var)) {
  	...
  	template <typename T>
	void call(void (*func)(T uval, ArgT var), T uValue ) {
		connect([=](ArgT var) {    // users a lambda for the callback
      		(*func)(uValue, var); 
		});
  	}
...
public:
	...
	void call(void (*func)()) {  
  	...
  	// Just call a user supplied function - no object - but with a User Defined value
  	template <typename T>
	void call(void (*func)(T uval), T uValue ) {
 	...
 	// connects a const member function to this spSignal
  	template <typename T>
	void call(T *inst, void (T::*func)() const) {
		 connect([=]() {    // users a lambda for the callback
      		(inst->*func)(); 
		});
  	}
