...
spDevISM330Base::spDevISM330Base()
{
...
float spDevISM330Base::read_accel_x()
{
    if (!_accelX)
    {
...
float spDevISM330Base::read_accel_y()
{
    if (!_accelY)
    {
        if (getAccel(&_accelData))
...
float spDevISM330Base::read_accel_z()
{
    if (!_accelZ)
    {
        if (getAccel(&_accelData))
...
float spDevISM330Base::read_gyro_x()
{
    if (!_gyroX)
    {
...
float spDevISM330Base::read_gyro_y()
{
    if (!_gyroY)
    {
        if (getGyro(&_gyroData))
...
float spDevISM330Base::read_gyro_z()
{
    if (!_gyroZ)
    {
        if (getGyro(&_gyroData))
...
float spDevISM330Base::read_temperature()
{
...
uint8_t spDevISM330Base::get_accel_data_rate()
{
    return _accel_data_rate;
}
void spDevISM330Base::set_accel_data_rate(uint8_t rate)
{
    _accel_data_rate = rate;
    if (_begun)
        setAccelDataRate(rate);
}
uint8_t spDevISM330Base::get_accel_full_scale()
{
    return _accel_full_scale;
}
void spDevISM330Base::set_accel_full_scale(uint8_t scale)
{
    _accel_full_scale = scale;
    if (_begun)
        setAccelFullScale(scale);
}
uint8_t spDevISM330Base::get_gyro_data_rate()
{
    return _gyro_data_rate;
}
void spDevISM330Base::set_gyro_data_rate(uint8_t rate)
{
    _gyro_data_rate = rate;
    if (_begun)
        setGyroDataRate(rate);
}
uint8_t spDevISM330Base::get_gyro_full_scale()
{
    return _gyro_full_scale;
}
void spDevISM330Base::set_gyro_full_scale(uint8_t scale)
{
    _gyro_full_scale = scale;
    if (_begun)
        setGyroFullScale(scale);
}
uint8_t spDevISM330Base::get_accel_filter_lp2()
{
    return (uint8_t)_accel_filter_lp2;
}
void spDevISM330Base::set_accel_filter_lp2(uint8_t enable)
{
    _accel_filter_lp2 = (bool)enable;
    if (_begun)
        setAccelFilterLP2((bool)enable);
}
uint8_t spDevISM330Base::get_gyro_filter_lp1()
{
    return (uint8_t)_gyro_filter_lp1;
}
void spDevISM330Base::set_gyro_filter_lp1(uint8_t enable)
{
    _gyro_filter_lp1 = (bool)enable;
    if (_begun)
        setGyroFilterLP1((bool)enable);
}
uint8_t spDevISM330Base::get_accel_slope_filter()
{
    return _accel_slope_filter;
}
void spDevISM330Base::set_accel_slope_filter(uint8_t slope)
{
    _accel_slope_filter = slope;
    if (_begun)
        setAccelSlopeFilter(slope);
}
uint8_t spDevISM330Base::get_gyro_lp1_bandwidth()
{
    return _gyro_lp1_bandwidth;
}
void spDevISM330Base::set_gyro_lp1_bandwidth(uint8_t bw)
{
...
// Place to initialized the underlying device library/driver
//
bool spDevISM330_SPI::onInitialize(SPIClass &spiPort)
{
