...
public:
	...
	virtual bool onInitialize(TwoWire& ){return true;};

	// Method called when a managed property is updated.
	virtual void onPropertyUpdate(const char *){};

	virtual spType * getType(void){ return (spType*)nullptr;}
...
public:
	...
	int factory_count(void){ return _Builders.size();};
...
public:
	...
	virtual bool isConnected(spDevI2C& i2cDriver)=0;  // used to determine if a device is connected
...
class DeviceBuilder: public spDeviceBuilder{
   ...
   public:
   	  ...
   	  bool isConnected(spDevI2C& i2cDriver){

   	  	  return DeviceType::isConnected( i2cDriver);  // calls device object static isConnected method()
      }

      const char * getDeviceName(void){               // Calls device objects static method.
