...
#include "spDevADS1015.h"


// The ADS1015 supports multiple addresses:
uint8_t spDevADS1015::defaultDeviceAddress[] = { 0x48, 0x49, 0x4A, 0x4B, kSparkDeviceAddressNull};

// Register this class with the system - this enables the *auto load* of this device
spRegisterDevice(spDevADS1015);

spDevADS1015::spDevADS1015()
{
    ...
    spRegister(differential_0_minus_1, "Differential: Channel 0 - Channel 1 (mV)", "Differential: Channel 0 - Channel 1 (millivolts)");
    spRegister(differential_0_minus_3, "Differential: Channel 0 - Channel 3 (mV)", "Differential: Channel 0 - Channel 3 (millivolts)");
    spRegister(differential_1_minus_3, "Differential: Channel 1 - Channel 3 (mV)", "Differential: Channel 1 - Channel 3 (millivolts)");
    spRegister(differential_2_minus_3, "Differential: Channel 2 - Channel 3 (mV)", "Differential: Channel 2 - Channel 3 (millivolts)");
}

// Function to encapsulate the ops needed to get values from the sensor.
float spDevADS1015::read_single_0()
{
    return (ADS1015::getSingleEndedMillivolts(0));
}
float spDevADS1015::read_single_1()
{
    return (ADS1015::getSingleEndedMillivolts(1));
}
float spDevADS1015::read_single_2()
{
    return (ADS1015::getSingleEndedMillivolts(2));
}
float spDevADS1015::read_single_3()
{
    return (ADS1015::getSingleEndedMillivolts(3));
}
float spDevADS1015::read_differential_P0_N1()
{
    return (ADS1015::getDifferentialMillivolts(ADS1015_CONFIG_MUX_DIFF_P0_N1));
}
float spDevADS1015::read_differential_P0_N3()
{
    return (ADS1015::getDifferentialMillivolts(ADS1015_CONFIG_MUX_DIFF_P0_N3));
}
float spDevADS1015::read_differential_P1_N3()
{
    return (ADS1015::getDifferentialMillivolts(ADS1015_CONFIG_MUX_DIFF_P1_N3));
}
float spDevADS1015::read_differential_P2_N3()
{
...
//
bool spDevADS1015::onInitialize(TwoWire &wirePort)
{
...
uint16_t spDevADS1015::get_sample_rate()
{
    if (_begun)
        _sampleRate = ADS1015::getSampleRate();
    return _sampleRate;
}
void spDevADS1015::set_sample_rate(uint16_t rate)
{
    _sampleRate = rate;
    if (_begun)
        ADS1015::setSampleRate(rate);
}
uint16_t spDevADS1015::get_pga_gain()
{
    if (_begun)
        _gain = ADS1015::getGain();
    return _gain;
}
void spDevADS1015::set_pga_gain(uint16_t gain)
{
    _gain = gain;
    if (_begun)
